...
public class Bishop extends Piece{
    ...
    public Bishop(final int pieceTile, final playerColor pieceColor) {
        super(PieceType.BISHOP, pieceTile, pieceColor, true);
    }

    public Bishop(final int pieceTile, final playerColor pieceColor, final boolean isFirstMove) {
        super(PieceType.BISHOP, pieceTile, pieceColor, isFirstMove);
    }
    @Override
    ...
    public Piece movePiece(final Move move) {
        return new Bishop(move.getDestinationCoordinate(), move.getPlayingPiece().getPieceColor());
    }

    private static boolean firstColumnPos(final int currentPosition, final int Possible_Move) {
        return generalBoardRules.firstColumn[currentPosition] && ((Possible_Move == -9) || (Possible_Move == 7));   
    }
    private static boolean eighthColumnPos(final int currentPosition, final int Possible_Move) {
        return generalBoardRules.eighthColumn[currentPosition] && ((Possible_Move == -7) || (Possible_Move == 9));
    }
}
