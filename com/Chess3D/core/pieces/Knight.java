package com.Chess3D.core.pieces;

import static com.Chess3D.core.board.generalBoardRules.*;

import java.util.ArrayList;
import java.util.Collections;
import java.util.List;

import com.Chess3D.core.playerColor;
import com.Chess3D.core.board.ChessBoard;
import com.Chess3D.core.board.Move;
import com.Chess3D.core.board.Move.normalMove;
import com.Chess3D.core.board.Tile;
import com.Chess3D.core.board.generalBoardRules;


public class Knight extends Piece{

    private final static int[] Possible_Moves = {-17, -15, -10, -6, 6, 10, 15, 17};

    public Knight(final int pieceTile, final playerColor pieceColor) {
        super( PieceType.KNIGHT, pieceTile, pieceColor, true);
    }

    public Knight(final int pieceTile, final playerColor pieceColor, final boolean isFirstMove) {
        super( PieceType.KNIGHT, pieceTile, pieceColor, isFirstMove);
    }
    
    @Override
    public List<Move> validMove(final ChessBoard board) {

        int possibleDestination;
        final List<Move> validMoves = new ArrayList<>();

        for (final int currentMove : Possible_Moves) {
            possibleDestination = this.pieceTile + currentMove;
            if (isValidTileCoordinate(possibleDestination)) {

                if (firstColumnPos(this.pieceTile, currentMove) || secondColumnPos(this.pieceTile, currentMove) || seventhColumnPos(this.pieceTile, currentMove) || eighthColumnPos(this.pieceTile, currentMove)) {
                    continue;
                }

                final Tile possibleDestinationTile = board.getTile(possibleDestination);
                if (!possibleDestinationTile.tileOccupation()){
                    validMoves.add(new normalMove(board, this, possibleDestination));
                }else{
                    final Piece pieceAtDestination = possibleDestinationTile.getPiece();
                    final playerColor pieceColorDestination = pieceAtDestination.getPieceColor();
                    if (this.pieceColor != pieceColorDestination){
                        validMoves.add(new Move.MajorAttackMove(board, this, possibleDestination, pieceAtDestination));
                    }
                }
            }
        }
        return Collections.unmodifiableList(validMoves);
    }

    @Override
    public String toString() {
        return pieceType.KNIGHT.toString();
    }

    @Override
    public Piece movePiece(final Move move) {
        return new Knight(move.getDestinationCoordinate(), move.getPlayingPiece().getPieceColor());
    }

    private static boolean firstColumnPos(final int currentPosition, final int Possible_Move) {
        return generalBoardRules.firstColumn[currentPosition] && ((Possible_Move == -17) || (Possible_Move == -10) || (Possible_Move == 6) || (Possible_Move == 15));   
    }

    private static boolean secondColumnPos(final int currentPosition, final int Possible_Move) {
        return generalBoardRules.secondColumn[currentPosition] && ((Possible_Move == -10) || (Possible_Move == 6));   
    }

    private static boolean seventhColumnPos(final int currentPosition, final int Possible_Move) {
        return generalBoardRules.seventhColumn[currentPosition] && ((Possible_Move == 10) || (Possible_Move == -6));
    }

    private static boolean eighthColumnPos(final int currentPosition, final int Possible_Move) {
        return generalBoardRules.eighthColumn[currentPosition] && ((Possible_Move == 10) || (Possible_Move == -6) || (Possible_Move == -15) || (Possible_Move == 17));
    }


}
